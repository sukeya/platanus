...
#ifndef PLATANUS_POLYMORPHIC_ALLOCATOR_H__
#define PLATANUS_POLYMORPHIC_ALLOCATOR_H__
...
class polymorphic_allocator {
 ...
 public:
  ...
  template <class U>
  polymorphic_allocator(const polymorphic_allocator<U>& other) noexcept : mem_res_ptr_(other.resource()) {}

  [[nodiscard]] Tp* allocate(std::size_t n) {
    constexpr std::size_t max = std::numeric_limits<std::size_t>::max();
    return static_cast<Tp*>(mem_res_ptr_->allocate((max / sizeof(Tp) < n) ? max : (n * sizeof(Tp)) , alignof(Tp)));
  }
  ...
  void deallocate_object(T* p, std::size_t n = 1) {
    mem_res_ptr_->deallocate_bytes(p, n * sizeof(T), alignof(T));
  }
...
bool operator==(const polymorphic_allocator<T1>& left, const polymorphic_allocator<T2>& right) noexcept {
