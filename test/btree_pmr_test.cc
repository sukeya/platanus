...
TEST(node_factory, leaf_root_node) {
  auto node_factory = pmr::details::btree_node_factory<
    commons::btree_set_params<int, std::ranges::less, std::pmr::polymorphic_allocator<>, 3>
  >();
...
TEST(node_factory, leaf_node) {
  ...
  auto root = node_factory.make_root_node(false);
  commons::set_child(root.get(), 0, node_factory.make_node(true, root.get()));
}
...
TEST(node_factory, internal_node) {
  ...
  auto root = node_factory.make_root_node(false);
  commons::set_child(root.get(), 0, node_factory.make_node(false, root.get()));
}
}  // namespace platanus
